...
module.exports = async function(
  ...
  const [ students, quarters ] = await Promise.all([
    pool.query(`
      select studentID, (select studentName from studentInfo where studentID.studentID=studentInfo.studentID) as studentName from studentID where studentID not in (select studentID from billing where date_format(current_date, '%Y-%m')=date_format(lessonMonth, '%Y-%m')) order by studentID`
    ),
    pool.query(`
