...
module.exports = async function(
  ...
  pool.query(`
    select case when count(*)>0 then 1 else 0 end as permission from lesson, lessonCharge where
    lesson.quarterID=lessonCharge.quarterID and
    lesson.lessonMonth=lessonCharge.lessonMonth and
    date_format(lessonCharge.lessonMonth, '%Y-%m')=date_format(?, '%Y-%m') and
    lessonCharge.teacherID=? and
    lesson.quarterID=?`,
