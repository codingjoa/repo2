...
const { setInfo } = require('../sessionMapManager');
...
module.exports = async function(
  ...
  pool.query(
    'select teacherID, teacherPassword, teacherOp from teacher where teacherAccount=? limit 1',
    [ teacherAccount ]
  )
  .then(async r => {
    ...
    if(req.session.tid === undefined) {
      ...
      setInfo(tid, { id: teacherAccount, op: r[0].teacherOp});
      req.session.tid = tid;
    }
    next();
  })
